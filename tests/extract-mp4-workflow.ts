...
async function extractWorkflowFromMp4(filePath: string): Promise<any> {
  ...
  while (offset < metaEnd) {
    ...
    console.log(
      `Inner meta box at offset ${offset}: Type ${type}, Size ${size}`,
    );
  ...
  console.log(
    `Looking for item ${workflowKeyIndex + 1} in ilst box at offset ${ilstOffset}`,
  );
  ...
  while (offset < ilstEnd) {
    ...
    console.log(
      `Item at offset ${offset}: Index ${itemIndex}, Size ${itemSize}`,
    );
    itemCount++;

  ...
  try {
    ...
    if (metadata.workflow) {
      ...
      try {
        ...
        await fs.writeFile('./extracted-workflow.json', JSON.stringify(workflow, null, 2));
        console.log('Saved extracted workflow to extracted-workflow.json');
        
        // Compare with reference file
        const referenceWorkflowPath = './tests/mp4/img2vid_00009_.mp4.workflow.json';
        const referenceWorkflow = JSON.parse(await fs.readFile(referenceWorkflowPath, 'utf-8'));
        
        console.log('Reference workflow ID:', referenceWorkflow.id);
        console.log('Reference node count:', referenceWorkflow.nodes?.length || 0);
        console.log('Reference link count:', referenceWorkflow.links?.length || 0);

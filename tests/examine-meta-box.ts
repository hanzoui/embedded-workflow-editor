...
async function examineMetaBox(filePath: string): Promise<void> {
  ...
  while (offset < udtaEnd) {
    ...
    if (type === 'meta') {
      ...
      if (offset + 12 <= udtaEnd) {
        ...
        const flags = (buffer[offset + 9] << 16) | (buffer[offset + 10] << 8) | buffer[offset + 11];
        console.log(`  Meta box version: ${version}, flags: 0x${flags.toString(16)}`);
        
        // Meta box typically contains an 'hdlr' box followed by other boxes
        let innerOffset = offset + 12;
        const metaEnd = offset + size;
        
        console.log(`  Meta box internal structure from ${innerOffset} to ${metaEnd}:`);
        while (innerOffset < metaEnd) {
          ...
          console.log(`    Inner box at offset ${innerOffset}: Type ${innerType}, Size ${innerSize}`);
          
          // Look for 'ilst' (item list) box which contains the actual metadata
          if (innerType === 'ilst') {
            console.log(`    Found ilst box - analyzing items from ${innerOffset + 8} to ${innerOffset + innerSize}:`);
            ...
            while (itemOffset < ilstEnd) {
              ...
              console.log(`      Item at offset ${itemOffset}: Name ${itemName}, Size ${itemSize}`);
              ...
              if (dataOffset + 8 <= itemOffset + itemSize) {
                ...
                if (dataBoxType === 'data' && dataOffset + 16 <= itemOffset + itemSize) {
                  ...
                  if (dataType === 1) {
                    ...
                    console.log(`        Text data: ${text.length > 100 ? text.substring(0, 100) + '...' : text}`);
                    ...
                    if (text.trim().startsWith('{') && text.trim().endsWith('}')) {
                      try {
                        ...
                        if (json.nodes && json.links) {
                          console.log('        This appears to be workflow data!');
                          // If it looks like workflow data, dump the first part only
                          console.log(`        Workflow excerpt: ${JSON.stringify(json).substring(0, 200)}...`);
                        }
                      } catch (e) {
                    ...
                    hexDump(buffer, dataOffset + 16, Math.min(64, dataBoxSize - 16));
                  }
                }
              }
            ...
            if (innerOffset + 16 <= metaEnd) {
              ...
              const flags = (buffer[innerOffset + 9] << 16) | (buffer[innerOffset + 10] << 8) | buffer[innerOffset + 11];
              const entryCount = buffer.readUInt32BE(innerOffset + 12);
              
              console.log(`      Keys version: ${version}, flags: 0x${flags.toString(16)}, entry count: ${entryCount}`);
              
              // Parse each key entry
              let keyOffset = innerOffset + 16;
              for (let i = 0; i < entryCount && keyOffset < innerOffset + innerSize; i++) {
                ...
                if (keyOffset + 8 + keyValueLength <= innerOffset + innerSize) {
                  const keyValue = buffer.subarray(keyOffset + 8, keyOffset + 8 + keyValueLength);
                  const keyName = new TextDecoder().decode(keyValue);
                  
                  console.log(`      Key ${i+1}: Namespace ${keyNamespace}, Name ${keyName}`);
                }
      ...
      console.log(`Found potential workflow box (wflo) at offset ${offset}, size ${size}`);
      if (offset + 12 < udtaEnd) {
        const version = buffer[offset + 8];
        const flags = (buffer[offset + 9] << 16) | (buffer[offset + 10] << 8) | buffer[offset + 11];
        console.log(`  Workflow box version: ${version}, flags: 0x${flags.toString(16)}`);
        
        // Data starts at offset + 12
        const dataOffset = offset + 12;
        const dataLength = size - 12;
        console.log(`  Workflow data from ${dataOffset} to ${dataOffset + dataLength}:`);
        ...
        try {
          const workflowData = new TextDecoder().decode(
            buffer.subarray(dataOffset, dataOffset + dataLength)
          ).trim();
          
          console.log(`  Workflow text data (first 200 chars): ${workflowData.substring(0, 200)}...`);
  ...
  if (metaOffset === 0) {
    console.log('No meta box found, looking for other possible metadata containers');
...
examineMetaBox('./tests/mp4/img2vid_00009_.mp4')
  .catch(error => console.error('Error analyzing MP4 meta box:', error));
